...
页面已移除模拟数据按钮，避免在没有真实传感器数据时误显示假数据。
...
- 最近一次数据区域显示心率、血氧、时间。
- 没有真实数据的字段显示 `----`。
- 如果没有有效心率数据，页面提示 `未检测到有效数据`，不会上传到后端。
- 如果有有效心率数据，页面提示 `正在上传健康数据`，上传成功后提示 `同步成功`。
- 回到首页后，心率卡片应更新。
...
12. 小程序自动上传数据。
13. 点击小程序 `连续监测`。
14. 页面显示 `连续监测已开启`，按钮变为 `停止监测`。
15. 手表约每 15 秒自动上报一次数据，小程序自动上传。
16. 点击 `停止监测` 后，页面显示 `连续监测已停止`。
...
```text
{"sys":null,"dia":null,"hr":72,"spo2":98,"battery":null}\n
```
