...
class MeasurementCreate(BaseModel):
    ...
    def normalize_legacy_fields(cls, data):
        ...
        if "type" not in normalized and "measurement_type" in normalized:
            normalized["type"] = normalized["measurement_type"]
        if "value1" not in normalized:
            for key in ("systolic", "blood_sugar", "heart_rate", "weight", "temperature"):
                if key in normalized:
                    normalized["value1"] = normalized[key]
                    break
        if "value2" not in normalized and "diastolic" in normalized:
...
class MessageCreate(BaseModel):
    ...
    @classmethod
    def normalize_legacy_fields(cls, data):
        if isinstance(data, dict) and "message_type" not in data and "type" in data:
            normalized = dict(data)
